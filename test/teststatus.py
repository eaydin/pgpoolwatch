...
if __name__ == '__main__':
    ...
    if not args.run_once:
        ...
        while True:
            ...
            #     if server_status:
            #         # this means the state hasn't changed
            #         # the server is up, and was up
            #         # so we don't inform anyone
            #         print("runpgpwatch and server_status is true - no state change")
            #         pass
            #     else:
            #         # this means the status is true
            #         # but the server_status was false
            #         # so the state has changed
            #         # we should inform the cylons
            #         if not check_port():
            #             server.start()
            #             if check_port():
            #                 print("Successfully Started")
            #                 send_mail(subject="PGPWATCH - Status Change - PGP Up", body="The PGPServer was down, now switching to <b>up</b> state. <br>Opening port for GSLB.")
            #                 server_status = True
            #             else:
            #                 print("Failed starting?")
            #                 server_status = False
            #         else:
            #             print("Port is already open, yet we thought server_status was False?")
            #             server_status = True
            #
            # else:
            #     # if the runpgpwatch returns False
            #     if not server_status:
            #         # this means the state hasn't changed
            #         # the server is down, and was down
            #         # assuming we've already informed cylons when the server went down
            #         # we don't do anything
            #         print("runpgpwatch and server_status is false - no state change")
            #         pass
            #     else:
            #         # this means the state has changed
            #         # so the runpgpwatch returned False
            #         # yet the port was open
            #
            #         # let's check if the port is really open
            #         if check_port():
            #             # yes the port is really open, let's close it
            #             server.stop()
            #             if not check_port():
            #                 # closing the port was successful
            #                 # let's inform the cylons about the state change
            #                 print("Successfully closed the port")
            #                 send_mail(subject="PGPWATCH - Status Change - PGP Down", body="The PGPServer was up, now it <b>failed</b>. <br>Closing port for GSLB.")
            #                 server_status = False
            #             else:
            #                 print("Failed closing port?")
            #                 server_status = True
            #         else:
            #             # the port is already closed
            #             print("Port is already closed, yet we thought server_status was True?")
            #             server_status = False
            ...
            if check_port():
                ...
                server.start()
            print("Instance running: {0}".format(n))
    else:
        if mail_on_success:
            runpgpwatch(True)
        else:
